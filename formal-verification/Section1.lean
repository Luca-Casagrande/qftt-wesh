...
PART VII:  Mathematical support — TraceNorm, characteristic polynomial lemmas

Lean 4.24.0 / Mathlib f897ebcf

AXIOM NOTE:
  eigenvalues_conjTranspose_mul_self_nonneg : eigenvalues of A*A ≥ 0
  Mathlib's Matrix.PosSemidef.eigenvalues_nonneg requires PartialOrder ℂ which doesn't exist.
  Sound axiom (eigenvalues of A*A = squared singular values).
-/
...
Supporting mathematical machinery including the trace norm (for density matrix
distance), and characteristic polynomial lemmas used in spectral analysis.
-/

open Matrix Complex BigOperators Real

variable {n : Type} [Fintype n] [DecidableEq n]

omit [DecidableEq n] in
/-- A*A is Hermitian. -/
lemma isHermitian_conjTranspose_mul_self_local (A : Matrix n n ℂ) : (A.conjTranspose * A).IsHermitian := by
  simp +decide [Matrix.IsHermitian, Matrix.conjTranspose_mul]

/-- Axiom: Eigenvalues of A*A are nonnegative.
    Mathematically obvious (eigenvalues = squared singular values).
    Mathlib requires PartialOrder ℂ which doesn't exist. -/
axiom eigenvalues_conjTranspose_mul_self_nonneg {n : Type} [Fintype n] [DecidableEq n]
  (A : Matrix n n ℂ) (hH : (A.conjTranspose * A).IsHermitian) (i : n) : 0 ≤ hH.eigenvalues i

/-- Trace norm (nuclear norm) ||A||_1 = Σ σ_i. -/
noncomputable def traceNorm (A : Matrix n n ℂ) : ℝ :=
  let hH := isHermitian_conjTranspose_mul_self_local A
  Finset.univ.sum fun i => Real.sqrt (hH.eigenvalues i)
...
/-- Trace norm is zero iff A is zero. -/
lemma traceNorm_eq_zero_iff (A : Matrix n n ℂ) : traceNorm A = 0 ↔ A = 0 := by
  constructor
  · intro h
    have h_eigenvalues_zero : ∀ i : n, (Matrix.IsHermitian.eigenvalues (isHermitian_conjTranspose_mul_self_local A) i) = 0 := by
      unfold traceNorm at h; intro i; contrapose! h; simp_all +decide [Finset.sum_eq_zero_iff_of_nonneg, Real.sqrt_nonneg]
      have h_eigenvalues_nonneg : ∀ i, 0 ≤ (Matrix.IsHermitian.eigenvalues (isHermitian_conjTranspose_mul_self_local A) i) := fun i => eigenvalues_conjTranspose_mul_self_nonneg A (isHermitian_conjTranspose_mul_self_local A) i
      generalize_proofs at *
      exact ⟨i, ne_of_gt <| Real.sqrt_pos.mpr <| lt_of_le_of_ne (h_eigenvalues_nonneg i) <| Ne.symm h⟩
    have h_A_star_A_zero : A.conjTranspose * A = 0 := by
      have h_A_star_A_zero : (A.conjTranspose * A).IsHermitian := by simp +decide [Matrix.IsHermitian, Matrix.conjTranspose_mul]
      have := Matrix.IsHermitian.spectral_theorem h_A_star_A_zero
      ext i j; replace this := congr_fun (congr_fun this i) j; simp_all +decide [Matrix.mul_assoc]
      simp_all +decide [Matrix.mul_apply, Matrix.diagonal]
    rw [← Matrix.ext_iff] at *
    simp_all +decide [Matrix.mul_apply, Complex.ext_iff]
    intro i j; specialize h_A_star_A_zero j j
    simp_all +decide [Finset.sum_eq_zero_iff_of_nonneg, add_nonneg, mul_self_nonneg]
    constructor <;> nlinarith only [h_A_star_A_zero.1 i]
  · rintro rfl; unfold traceNorm; generalize_proofs at *; norm_num [Matrix.IsHermitian.eigenvalues_eq]

/-- Scaling of A*A under scalar multiplication. -/
lemma conjTranspose_mul_self_smul (c : ℂ) (A : Matrix n n ℂ) :
    (c • A).conjTranspose * (c • A) = (Complex.normSq c) • (A.conjTranspose * A) := by
  simp +decide []
  simp +decide [Complex.mul_conj, Complex.normSq_eq_norm_sq, smul_smul]
  norm_cast

omit [DecidableEq n] in
/-- If v is an eigenvector of A with eigenvalue μ, then v is an eigenvector of r•A with eigenvalue r*μ. -/
lemma has_eigenvector_smul (A : Matrix n n ℂ) (v : n → ℂ) (mu : ℝ) (r : ℝ)
  (h : A *ᵥ v = (mu : ℂ) • v) :
  ((r : ℂ) • A) *ᵥ v = ((r * mu) : ℂ) • v := by
    rw [Matrix.smul_mulVec, h, smul_smul]

open Polynomial

/-- Characteristic polynomial of scaled matrix. -/
lemma charpoly_smul_eq (c : ℂ) (hc : c ≠ 0) (A : Matrix n n ℂ) :
  (c • A).charpoly = (charpoly A).comp (C (c⁻¹) * X) * C (c ^ Fintype.card n) := by
    rw [Matrix.charpoly, Matrix.charpoly]
    have h_det : Matrix.det (Matrix.diagonal (fun _ => Polynomial.X) - Polynomial.C (c : ℂ) • Matrix.map A (Polynomial.C)) = Matrix.det (Polynomial.C (c : ℂ) • (Matrix.diagonal (fun _ => Polynomial.C (1 / c) * Polynomial.X) - Matrix.map A (Polynomial.C))) := by
      congr; ext i j; by_cases hij : i = j <;> simp +decide [hij, hc, mul_sub]
    convert h_det using 1
    · simp +decide [Matrix.charmatrix, Matrix.smul_eq_diagonal_mul]
    · simp +decide
      rw [mul_comm]
      simp +decide [Matrix.det_apply']
      simp +decide [Polynomial.comp, Polynomial.eval₂_finset_prod]
      exact Or.inl (Finset.sum_congr rfl fun _ _ => by congr; ext; by_cases hi : (‹Equiv.Perm n› : n → n) ‹_› = ‹_› <;> simp +decide [hi])

noncomputable section CharpolyLemmas

open Polynomial

/-- Roots of p.comp(C a * X) = roots of p scaled by a⁻¹. -/
lemma roots_comp_C_mul_X {K : Type*} [Field K] (p : K[X]) (a : K) (ha : a ≠ 0) :
  (p.comp (C a * X)).roots = p.roots.map (fun z => a⁻¹ * z) := by
    ext x
    rw [Multiset.count_map]
    rw [show (Multiset.filter (fun y => x = a⁻¹ * y) p.roots) = Multiset.filter (fun y => a * x = y) p.roots from ?_]
    · by_cases hx : p.roots.count (a * x) = 0 <;> simp_all +decide
      · rw [Multiset.filter_eq_nil.mpr] <;> aesop
      · rw [show (Multiset.filter (fun y => a * x = y) p.roots) = Multiset.filter (fun y => y = a * x) p.roots by simp +decide only [eq_comm]]
        rw [show (Multiset.filter (fun y => y = a * x) p.roots) = Multiset.replicate (Polynomial.rootMultiplicity (a * x) p) (a * x) from ?_]
        · have h_div : ∀ k : ℕ, (Polynomial.X - Polynomial.C x) ^ k ∣ p.comp (Polynomial.C a * Polynomial.X) ↔ (Polynomial.X - Polynomial.C (a * x)) ^ k ∣ p := by
            intro k
            constructor
            intro h_div
            have h_div' : (Polynomial.X - Polynomial.C (a * x)) ^ k ∣ p := by
              obtain ⟨q, hq⟩ := h_div
              have h_sub : p.comp (Polynomial.C a * (Polynomial.C (1 / a) * Polynomial.X)) = (Polynomial.C (1 / a) * Polynomial.X - Polynomial.C x) ^ k * q.comp (Polynomial.C (1 / a) * Polynomial.X) := by
                convert congr_arg (Polynomial.comp · (Polynomial.C (1 / a) * Polynomial.X)) hq using 1 <;> simp +decide [Polynomial.comp_assoc]
              have h_sub' : p = (Polynomial.C (1 / a) * Polynomial.X - Polynomial.C x) ^ k * q.comp (Polynomial.C (1 / a) * Polynomial.X) := by
                convert h_sub using 1
                simp +decide [← mul_assoc, ← Polynomial.C_mul, ha]
              rw [h_sub']
              refine' dvd_mul_of_dvd_left _ _
              refine' pow_dvd_pow_of_dvd _ _
              exact Polynomial.dvd_iff_isRoot.mpr (by simp +decide [ha])
            exact h_div'
            rintro ⟨q, rfl⟩
            field_simp
            rw [Polynomial.comp, Polynomial.eval₂_mul, Polynomial.eval₂_pow, Polynomial.eval₂_sub, Polynomial.eval₂_X, Polynomial.eval₂_C]
            exact dvd_mul_of_dvd_left (by rw [show (Polynomial.C a * Polynomial.X - Polynomial.C (a * x)) = (Polynomial.X - Polynomial.C x) * Polynomial.C a by simpa using by ring]; rw [mul_pow]; exact dvd_mul_right _ _) _
          ...
          · simp +decide [emultiplicity, h_div]
            simp +decide [FiniteMultiplicity, h_div]
        · ext y; rw [Multiset.count_replicate]; aesop
    · field_simp

end CharpolyLemmas

lemma roots_charpoly_smul (c : ℂ) (hc : c ≠ 0) (A : Matrix n n ℂ) :
  (charpoly (c • A)).roots = (charpoly A).roots.map (fun z => c * z) := by
    have h_roots : (c • A).charpoly = (charpoly A).comp (C (c⁻¹) * X) * C (c ^ Fintype.card n) := by
      exact charpoly_smul_eq c hc A
    generalize_proofs at *
    rw [h_roots, Polynomial.roots_mul]
    · rw [roots_comp_C_mul_X] <;> aesop
    · norm_num [hc, Polynomial.comp_eq_zero_iff]
      exact Matrix.charpoly_monic A |> fun h => h.ne_zero

open Polynomial

/-- Characteristic polynomial of Hermitian matrix splits into linear factors. -/
lemma charpoly_eq_prod_X_sub_C_eigenvalues (A : Matrix n n ℂ) (hA : A.IsHermitian) :
  charpoly A = Finset.univ.prod (fun i => X - C (hA.eigenvalues i : ℂ)) := by
    have := Matrix.charpoly_degree_eq_dim A
    norm_num +zetaDelta at *
    have h_charpoly : A.charpoly = Polynomial.C 1 * Finset.prod Finset.univ (fun i => Polynomial.X - Polynomial.C (hA.eigenvalues i : ℂ)) := by
      convert Matrix.charpoly_degree_eq_dim A
      constructor <;> intro h
      · rw [Matrix.charpoly_degree_eq_dim]
      · convert Matrix.IsHermitian.charpoly_eq hA
        norm_num
    aesop
