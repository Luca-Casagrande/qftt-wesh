...
/-
J.0 Doob-Meyer Decomposition: The eigentime counting process N_Eig(s) decomposes as
∫₀ˢ (Γ[Ψ(u)]/τ_Eig)du + M(s), where M(s) is a martingale.
Here Γ/τ_Eig is the instantaneous intensity (rate of eigentime events per unit s).
-/
...
structure EigentimeSetup (Ω : Type*) [MeasurableSpace Ω] (μ : Measure Ω) where
  ...
  -- Assumptions
  τ_Eig_pos : 0 < τ_Eig
  Γ_integrable : ∀ s, IntegrableOn (fun u => Γ (Ψ u) / τ_Eig) (Set.Ioc 0 s) volume
  ...
  -- M(s) = N_Eig(s) - ∫₀ˢ (Γ[Ψ(u)]/τ_Eig)du is a martingale
  -- The intensity Λ(s) := Γ[Ψ(s)]/τ_Eig is the compensator density
  martingale_M : Martingale (fun s ω => N_Eig s ω - ∫ u in Set.Ioc 0 s, Γ (Ψ u) / τ_Eig) ℱ μ
...
theorem J0_DoobMeyer (Ω : Type*) [MeasurableSpace Ω] (μ : Measure Ω)
  ...
  let Λ := fun u => S.Γ (S.Ψ u) / S.τ_Eig  -- instantaneous intensity
  let Compensator := fun s => ∫ u in Set.Ioc 0 s, Λ u
  let M := fun s ω => S.N_Eig s ω - Compensator s
  Martingale M S.ℱ μ := by
  intros Λ Compensator M
