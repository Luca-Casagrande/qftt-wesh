...
1. **Lemma D.1** (Markov–Kakutani fixed point: existence of stationary state)
...
4. **Proposition D.2** (Stationarity ⇒ alignment: PRD-level derivation, 7 steps)
...
- Zero sorries
- Axioms only for:
  (1) classical literature (Markov-Kakutani, Euler-Lagrange, Γ-convergence)
...
T is affine iff T(λx + (1-λ)y) = λT(x) + (1-λ)T(y) for all λ ∈ [0,1]. -/
def IsAffineMap {E : Type*} [AddCommGroup E] [Module ℝ E] (T : E → E) : Prop :=
  ∀ (x y : E) (t : ℝ), 0 ≤ t → t ≤ 1 →
    T (t • x + (1 - t) • y) = t • T x + (1 - t) • T y
...
/-- **Markov–Kakutani fixed point theorem** (Markov 1936, Kakutani 1938).

Standard functional-analytic result:
Let S be a nonempty compact convex subset of a locally convex space.
If {T_i} is a commuting family of **affine** **σ-continuous** maps S → S,
then there exists a common fixed point.

In the paper (Lemma D.1): The WESH semigroup {T_s} acts on S_phys which is
weak-* compact (Banach-Alaoglu) and convex. Each T_s is:
- Affine (CPTP maps preserve convex combinations)
- σ-continuous (weak-* continuous on trace-class)
- S_phys-invariant (preserves physical state constraints)
-/
axiom markov_kakutani_fixed_point
    ...
    (h_convex : Convex ℝ S)
    (T : ι → E → E)
    (h_affine : ∀ i, IsAffineMap (T i))  -- NEW: affine maps
    (h_cont : ∀ i, IsWeakStarContinuous (T i))  -- NEW: σ-continuous
    (h_maps : ∀ i, Set.MapsTo (T i) S S)
    (h_comm : ∀ i j x, T i (T j x) = T j (T i x)) :
    ∃ x ∈ S, ∀ i, T i x = x

/-- **Lemma D.1**: Existence of stationary state via Markov–Kakutani.

The WESH semigroup {T_s}_{s≥0} is a commuting family of **affine σ-continuous**
maps on S_phys (which is weak-* compact and convex).
By Markov-Kakutani, there exists ρ* fixed by ALL T_s, hence L[ρ*] = 0.

From the paper:
- S_phys is weak-* compact (Banach-Alaoglu on trace-class)
- Each T_s = e^{sL} is CPTP hence affine
- CPTP maps are σ-continuous on trace-class operators
-/
theorem lemma_D_1_markovKakutani
    {α : Type*}
    (Q : α → H →L[ℂ] H) (c : α → ℂ)
    (T : NNReals → (H →L[ℂ] H) → (H →L[ℂ] H))
    (L : (H →L[ℂ] H) → (H →L[ℂ] H))
    (h_compact : IsCompact (S_phys Q c))  -- weak-* compact in infinite dim
    (h_convex : Convex ℝ (S_phys Q c))
    (h_nonempty : (S_phys Q c).Nonempty)
    (h_affine : ∀ s : NNReals, IsAffineMap (T s))  -- T_s = e^{sL} is CPTP hence affine
    (h_cont : ∀ s : NNReals, IsWeakStarContinuous (T s))  -- σ-continuous
    (h_invariant : ∀ s : NNReals, Set.MapsTo (T s) (S_phys Q c) (S_phys Q c))
    (h_comm : ∀ s t : NNReals, ∀ ρ, T s (T t ρ) = T t (T s ρ))
    (h_fixed_implies_gen_zero : ∀ ρ, (∀ s : NNReals, T s ρ = ρ) → L ρ = 0) :
    ∃ ρ_star,
      ρ_star ∈ S_phys Q c ∧
      (∀ s : NNReals, T s ρ_star = ρ_star) ∧
      L ρ_star = 0 := by
  obtain ⟨ρ_star, hρ_mem, hρ_fix⟩ := markov_kakutani_fixed_point
    (S := S_phys Q c)
    h_nonempty h_compact h_convex T h_affine h_cont h_invariant
    (fun s t x => h_comm s t x)
