...
This project request had uuid: 5b2d22d5-8788-497a-a145-01ef617abacd
-/

/-
Formalization of Theorem D.2 (Variational Alignment) and Proposition D.2 (7-step derivation) from Appendix D.
Includes:
- Gradients of local, bilocal, and regularization parts of the Lyapunov functional.
- Decomposition of the Lyapunov functional.
- Total gradient of the Lyapunov functional.
- Proof that stationarity and hidden sector cancellation imply Einstein's equations (Proposition D.2).
- Proof of existence and uniqueness of the stationary state satisfying alignment and Einstein's equations (Theorem D.2).
-/
    ...
    HasFDerivAt (lyapunov_functional_loc T) (trace_re_map (A_loc T rho)) rho := by
      ...
      have h_deriv_term : ∀ x, HasFDerivAt (fun rho : H →L[ℂ] H => ((LinearMap.trace ℂ H (T x ^ 4 * rho)).re - (LinearMap.trace ℂ H (T x ^ 2 * rho)).re ^ 2)) (trace_re_map (T x ^ 4 - 2 * (LinearMap.trace ℂ H (T x ^ 2 * rho)).re • (T x ^ 2))) rho := by
        ...
        have h_local_term : HasFDerivAt (fun rho : H →L[ℂ] H => ((LinearMap.trace ℂ H (T x ^ 4 * rho)).re)) (trace_re_map (T x ^ 4)) rho ∧ HasFDerivAt (fun rho : H →L[ℂ] H => ((LinearMap.trace ℂ H (T x ^ 2 * rho)).re)) (trace_re_map (T x ^ 2)) rho := by
          ...
          · unfold trace_re_map;
            simp +decide [ mul_add, add_mul, pow_succ ];
            exact?;
          · unfold trace_re_map;
            simp +decide [ mul_add, add_mul, mul_assoc, mul_left_comm, LinearMap.map_add, LinearMap.map_smul, ContinuousLinearMap.coe_coe ];
            exact?;
        convert h_local_term.1.sub ( h_local_term.2.pow 2 ) using 1 ; ext ; simp +decide [ mul_assoc, pow_two ] ; ring;
        unfold trace_re_map; simp +decide [ mul_assoc, mul_comm, mul_left_comm ] ; ring;
        unfold trace_clm mul_left_clm; simp +decide [ mul_assoc, mul_comm, mul_left_comm ] ; ring;
        erw [ two_mul ] ; simp +decide [ mul_assoc, mul_comm, mul_left_comm ] ; ring;
      simp +decide [ A_loc ];
      convert HasFDerivAt.sum fun x _ => h_deriv_term x using 1;
      any_goals exact Finset.univ;
      · ext; simp +decide [ Finset.sum_sub_distrib ] ;
      · ext; simp +decide [ mul_assoc, mul_left_comm, Finset.mul_sum _ _ _, Finset.sum_mul, trace_re_map ] ;
        simp +decide [ mul_sub, sub_mul, Finset.sum_sub_distrib, mul_assoc, mul_left_comm, Finset.mul_sum _ _ _, Finset.sum_mul, trace_clm, mul_left_clm ]

#check lyapunov_gradient_bi_1
...
theorem lyapunov_gradient_reg
    ...
    HasFDerivAt (lyapunov_functional_reg epsilon) (trace_re_map ((2 * epsilon) • rho)) rho := by
      ...
      have h_trace_prod : ∀ x' : H →L[ℂ] H, (LinearMap.trace ℂ H) (x' * x') = (LinearMap.trace ℂ H) (rho * rho) + 2 * (LinearMap.trace ℂ H) (rho * (x' - rho)) + (LinearMap.trace ℂ H) ((x' - rho) * (x' - rho)) := by
        intro x';
        simp +decide [ two_mul, mul_sub, sub_mul, mul_add, add_mul, add_assoc, add_left_comm, add_comm, mul_assoc, mul_comm, mul_left_comm, LinearMap.trace_mul_comm ];
        ring;
        ...
        have h_norm_sq : ‖(LinearMap.trace ℂ H) ((x' - rho) * (x' - rho))‖ ≤ Module.finrank ℂ H * ‖(x' - rho) * (x' - rho)‖ := by
          have h_norm_sq : ∀ (A : H →L[ℂ] H), ‖(LinearMap.trace ℂ H) A‖ ≤ Module.finrank ℂ H * ‖A‖ := by
            ...
            have h_norm_sq : ‖(LinearMap.trace ℂ H) A‖ ≤ ∑ i : Fin (Module.finrank ℂ H), ‖A (OrthonormalBasis.toBasis (stdOrthonormalBasis ℂ H) i)‖ := by
              ...
              simpa using norm_inner_le_norm ( ( stdOrthonormalBasis ℂ H ).toBasis i ) ( A ( ( stdOrthonormalBasis ℂ H ).toBasis i ) ) |> le_trans <| by simp +decide [ norm_smul, OrthonormalBasis.orthonormal ] ;
      ...
      have h_bound : ∀ x' : H →L[ℂ] H, ‖epsilon * ((LinearMap.trace ℂ H) (x' * x')).re - epsilon * ((LinearMap.trace ℂ H) (rho * rho)).re - (Complex.reCLM.comp (ContinuousLinearMap.restrictScalars ℝ (trace_clm.comp (mul_left_clm ((2 * epsilon) • rho))))) (x' - rho)‖ ≤ ‖epsilon‖ * ‖x' - rho‖^2 * (Module.finrank ℂ H) := by
        ...
        have h_bound : ‖epsilon * ((LinearMap.trace ℂ H) (x' * x')).re - epsilon * ((LinearMap.trace ℂ H) (rho * rho)).re - (Complex.reCLM.comp (ContinuousLinearMap.restrictScalars ℝ (trace_clm.comp (mul_left_clm ((2 * epsilon) • rho))))) (x' - rho)‖ = ‖epsilon * ((LinearMap.trace ℂ H) ((x' - rho) * (x' - rho))).re‖ := by
          simp +decide [ h_trace_prod x', mul_assoc, mul_comm, mul_left_comm ];
          simp +decide [ mul_add, add_mul, mul_sub, sub_mul, mul_assoc, mul_comm, mul_left_comm, trace_clm, mul_left_clm ];
          rw [ ← abs_mul ] ; ring!;
...
theorem theorem_D_2_variational_alignment
    ...
    : ∃! rho_inf, is_state rho_inf ∧
                  ...
                  alignment_holds T gamma C rho_inf ∧
                  satisfies_einstein_equations T gamma G epsilon rho_inf := by
                    have := theorem_5_4_einstein_emergence H_eff T nu gamma C G epsilon axioms Phi eps_contract h_contract h_maps_to h_equiv_flow h_equiv_stat h_nu h_gamma h_C_pos;
                    convert this using 2;
                    constructor <;> intro h <;> simp_all +decide [ and_assoc ];
                    exact axioms.thm_D_1 _ ( h_equiv_stat _ h.1 |>.1 h.2.1 )