...
\]
without thermal ansatz. The construction has no free dimensionless parameters; deviations from the $1/4$ prefactor, or the $N^2$ scaling would falsify the framework.
\vspace{0.8cm}
...
Sections 1, 5, 6 and Appendices A--D and F--J have been formally verified in Lean 4/Mathlib.
}
\end{center}
\end{abstract}
...
\vspace{0.4em}
\textbf{Empirical targets (core vs.\ diagnostic).}
\begin{itemize}[leftmargin=1.2em,itemsep=0.25em]
\item \emph{Core: collective stability.} $\tau_{\mathrm{coh}}\!\propto\!N^2$ — a structural discriminant of the $N^{-2}$ bilocal weighting in the WESH channel.
\item \emph{Diagnostic: angular modulation.} Decoherence $\!\propto\!(1+\varepsilon\cos^2\theta)$, with a state-- and geometry--dependent $\varepsilon$; the expected $W$-state anti-modulation ($\varepsilon\!<\!0$) provides a direct control for state dependence.
\item \emph{Core: BH sector (IR bridge).} A near-horizon KMS fixed point yields
...
remainder of this section we derive quantitative pre-geometric predictions and
...
(ii)~a $\cos^2\theta$ angular modulation in parity decay, which provides a diagnostic test of state/geometry factorization and causal connectivity. 
Both follow from the WESH master equation and its causal kernel, and they provide quantitative targets for the simulations and hardware tests of Sec.~\ref{sec:experimental}.}
...
is not a fit choice but the condition that keeps the total bilocal collapse power
...
in the framework), this yields the collective coherence enhancement
\begin{equation*}
\tau_{\mathrm{coh}}(N)\propto N^2 ,
\end{equation*}
...
\varepsilon=\frac{a_2}{a_0}\,\frac{G_2}{G_0},
\end{equation*}
(Eq.~\eqref{eq:angular-law}) is derived in Appendix~F from (i) rotated-parity projection,
(ii) lattice-weighted geometry, and (iii) weak even gate anisotropy. Importantly, the
...
\end{enumerate}
Appendix~F further clarifies that an angle-independent baseline (hardware noise) can coexist with
...
state-coupled angular signature derived in Appendix~F.
\end{enumerate}

\paragraph{Status of the collision model.}
The collision model used in Sec.~3 is a deliberately reduced \emph{surrogate}:
it does not attempt to simulate the full WESH master equation (e.g.\ the full causal-kernel
structure or the pre-geometric WESH--Noether constraint), but isolates the specific ingredient
tested there---inverse scaling of the effective couplings implied by Eq.~\eqref{eq:cs-scaling}---in a
...
local benchmark returns $\alpha_\gamma=-1$), i.e.\ trending toward the WESH asymptote $\alpha_\gamma=-2$
...
for the collective-stability mechanism and for the scaling-inference pipeline; it is not, by itself,
standalone empirical evidence.
...
collective-stability discriminant require higher-fidelity multipartite entanglement at larger $N$. A natural
next step is to repeat the same control stack (state swap, geometry control, gate-matched fake-GHZ) on platforms
with cleaner and more characterizable noise models and reconfigurable connectivity (e.g., trapped-ion or neutral-atom
architectures), where scaling regimes beyond current superconducting hardware become experimentally accessible.
\vspace{0.8cm}
...
\medskip
\noindent Table~\ref{tab:framework-comparison} compares these approaches along four axes: treatment of time, collapse mechanism, arrow of time, and status of the Bekenstein--Hawking entropy.
...
\textbf{QFTT--WESH} & \textbf{Emergent} via eigentimes & \textbf{State-dependent} $\Gtime$ & \textbf{Intrinsic} & \textbf{Derived} (Sec.~6)\\
\bottomrule
\end{tabularx}
\caption{Comparison of selected approaches to the problem of time and black-hole entropy. Among the frameworks listed, QFTT--WESH is the only one that starts from a timeless Wheeler--DeWitt constraint and derives both Einstein dynamics and $S_{\rm BH}$ within a single chain, without postulating an area/entropy law or a microscopic time as independent inputs.}
...
\begin{remark}[Empirical targets at the fixed point]
The fixed point entails: (a) $\tau_{\rm coh}(N)\propto N^2$ (structural discriminant);
(b) an angular modulation $\Gamma(\theta)\propto 1+\varepsilon\cos^2\theta$ (diagnostic of state/geometry factorization);
(c) $\mathcal O(1/N)$ corrections to Einstein's equation (see Eq.~\eqref{eq:emergent-einstein}) as a future precision test.
The first two items are accessible to current NISQ platforms; the third awaits higher-precision regimes.
\end{remark}
\vspace{0.3cm}
\endgroup
...
\]
\textit{IBM Eagle subset.} With three representative orientations \((\alpha_h,\alpha_v,\alpha_d)=(90^\circ,0^\circ,\alpha_3)\),
\[
...
\noindent\emph{Corrections.} Higher even harmonics in \(C(\Psi)\) (e.g.\ \(a_4\mu^4\)) and next order in~\eqref{eq:G-grad} add small \(\cos(4\theta)\) admixtures at high precision; environmental channels add an angle‑independent baseline.\\
\medskip

\noindent\emph{Scaling.} Device dependence enters only via \(G_2/G_0\); \(\overline\Gamma\) inherits the \(\xi^5\) scaling.
\medskip

\noindent\emph{Cross-platform outlook.} Trapped-ion architectures (linear chains or effectively all-to-all coupling graphs) yield pair-orientation statistics—and thus $G_2/G_0$—that differ from heavy-hex superconducting layouts (and may require allowing a phase offset if the layout lacks reflection symmetry, i.e.\ $G_2^\perp\neq 0$). A dedicated validation on such platforms would test the predicted $\cos^2\theta$ harmonic independently of superconducting device geometry. This direction was left for future work.
\endgroup
\vspace{1cm}
...
\begin{enumerate}[leftmargin=1.2em,itemsep=0.25em]
\item \textbf{Core: collective stability.} The coherence time scales as $\tau_{\mathrm{coh}}(N)\propto N^2$, reflecting causal mixing under the WESH constraint.
...
\item \textbf{Core: BH thermodynamics/holography.} Near-horizon KMS structure yields the thermodynamic law (area term and logarithmic corrections) without external parameters; the framework establishes WESH validity through a holographic criterion spanning ${\sim}50$ orders of magnitude in radius, from Planck-scale correlations to macroscopic event horizons.
\end{enumerate}
Each item probes a different layer of the construction: collective stability and black-hole thermodynamics test core structural aspects of WESH (including the IR bridge to gravity), while the angular dependence provides a diagnostic check of causal connectivity and state/geometry factorization.
\vspace{0.3cm}
